...
- 2026-02-18: Task 8.3 cleanup tightened route/middleware type hints in `web2api/main.py`, added missing public constructor docstrings in `web2api/pool.py` and `web2api/registry.py`, and backpressure passed (`ruff check . --fix && ruff format .`; `pytest tests/unit tests/integration --timeout=30 -x -q` => `31 passed, 1 skipped`).
